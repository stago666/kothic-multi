...
<form class="flow_hidden" action="', $scripturl, '?action=pm;sa=pmactions;', $context['display_mode'] == 2 ? 'conversation;' : '', 'f=', $context['folder'], ';start=', $context['start'], $context['current_label_id'] != -1 ? ';l=' . $context['current_label_id'] : '', '" method="post" accept-charset="', $context['character_set'], '" name="pmFolder">';
	...
	if ($context['get_pmessage']('message', true))
	{
		...
		while ($message = $context['get_pmessage']('message'))
		{
			...
			$window_class2 = $message['alternate'] == 0 ? 'windowbg2' : 'windowbg';
			...
			if (!empty($message['is_replied_to']))
				...
				<h5 id="subject_', $message['id'], '" class="pm_sub">
						', $message['subject'], '
					</h5>
			</div>
				<br class="clear" />
	<div class="content_replies">
				...
				echo '
								<div style="float: left; padding: 0 5px;">
									<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">
										<img src="', $message['member']['avatar']['href'], '" style="vertical-align: middle; width: 40px; height: 40px;" alt="" />
									</a>
								</div>';
			else
				echo '
								<div style="float: left; padding: 0 5px;">
									<a href="', $scripturl, '?action=profile;u=', $message['member']['id'], '">
										<img src="', $settings['images_url'], '/no_avatar.png" style="vertical-align: middle; width: 40px; height: 40px;" alt="" />
									</a>
								</div>';
				if ($message['member']['online']['is_online']) {
					echo '
							<ul class="reset"><li class="status"><span class="online" style="text-transform: uppercase; font-size: 8px; text-shadow: 0 1px 0 #fff; color: #6DA26B;"> ' . $message['member']['online']['text'] . '</span></li></ul>';
				}
				else 
				{
					echo '
							<ul class="reset"><li class="status"><span class="offline" style="text-transform: uppercase; font-size: 8px; text-shadow: 0 1px 0 #fff; color: #AEAEAF;"> ' . $message['member']['online']['text'] . '</span></li></ul>';
				}
			...
			echo '
			<ul class="reset smalltext quickbuttons" style="float: right;">';
			...
			if (!empty($modSettings['onlineEnable']) && !$message['member']['is_guest'])
				echo '
				<img src="', $message['member']['online']['image_href'], '" alt="', $message['member']['online']['text'], '" class="online_status" />';
					echo '
										<strong>'.$message['member']['link'].'</strong>
										  ', $txt['on'], ': ', $message['time'], ' <a id="msg', $message['id'], '"></a>
									 </div>
								</div>
								<div class="re_coment ', $window_class2, '">
									 ',$message['body'];
									 
					echo '
					 
				', (!empty($modSettings['enableReportPM']) && $context['folder'] != 'sent' ? '<div class="adminreport"><a href="' . $scripturl . '?action=pm;sa=report;l=' . $context['current_label_id'] . ';pmsg=' . $message['id'] . '"><img src="' .$settings['images_url'] . '/report.png" alt="' . $txt['pm_report_to_admin'] . '" title="' . $txt['pm_report_to_admin'] . '" /></a></div>' : '');
