...
function template_main()
{
	...
	while ($message = $context['get_message']())
	{
		...
		echo '
				<div class="', $message['approved'] ? ($message['alternate'] == 0 ? 'posterbg' : 'posterbg') : 'approvebg', '">
					...
					<div class="poster_top">
							<div class="flow_hidden">
								<div class="keyinfo">
									...
									<div class="info_post">&#171; <strong>', !empty($message['counter']) ? $txt['reply_noun'] . ' #' . $message['counter'] : '', ' ', $txt['on'], ':</strong> ', $message['time'], ' &#187;</div>
		...
		echo '
							...
							<div class="b_quickbuttons"><div class="bnt_quickmenu"><img src="' . $settings['images_url'] . '/theme/quickmenu.png" class="btn_quickmenu_' . $message['id'] . '" alt="',$txt['open_quick'],'" title="',$txt['open_quick'],'" /></div></div>';
									// If this is the first post, (#0) just say when it was posted - otherwise give the reply #.
		if ($message['can_approve'] || $context['can_reply'] || $message['can_modify'] || $message['can_remove'] || $context['can_split'] || $context['can_restore_msg'])
			echo '
				<div class="quick_button_' . $message['id'] . '" style="display:none;">
			...
			echo '
								...
								<script type="text/javascript">
											$(document).ready(function(){	
													$(".btn_quickmenu_' . $message['id'] . '").click(function(){
														$(".quick_button_' . $message['id'] . '").slideToggle("fast");
													});
											});
								</script>';
		...
		if (!$message['member']['is_guest'])
		{
			...
			if ($modSettings['karmaMode'] == '1')
				echo '
								<span>', $modSettings['karmaLabel'], ' ', $message['member']['karma']['good'] - $message['member']['karma']['bad'], '</span>';
		...
		if (!empty($message['member']['title']))
			echo '
								<span>', $message['member']['title'], '</span>';
			echo'
			...
			echo '
				<div class="send_pm" style="float: left; margin-top: -2px; margin-bottom: -2px; margin-left: -1px;"><a href="', $scripturl, '?action=pm;sa=send;u=', $message['member']['id'], '"><span class="send_mp_new">', $txt['profile_sendpm_short'], '</span></a></div>';
				
			if ($message['member']['online']['is_online']) {
					echo '
							<span class="as_status" style="text-transform: uppercase; font-size: 8px; text-shadow: 0 1px 0 #fff; color: #6DA26B;"> ' . $message['member']['online']['text'] . '</span>';
				}
				else 
				{
					echo '
							<span class="as_status" style="text-transform: uppercase; font-size: 8px; text-shadow: 0 1px 0 #fff; color: #AEAEAF;"> ' . $message['member']['online']['text'] . '</span>';
				}
				echo'
							</div>
							<div class="as_divider"></div>
